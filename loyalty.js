function allFunc() {
        ...
        function lowtoHigh() {
            for (var i = 0; i < selectMembers.length; i++) {
                ///from lowest attendence (high missed vote pct)
        ...
        function tenPctFunction(array) {
            ...
            for (var i = 0; i < array.length; i++) {
                if (array[i].votes_with_party_pct === tenPctValueLow.votes_with_party_pct) {
        ...
        function createVoteTable(array1, target) {
            ...
            for (var i = 0; i < array1.length; i++) {
                ...
                var partyVotes = array1[i].total_votes * array1[i].votes_with_party_pct / 100;

                selectMissedVotes.innerHTML = partyVotes.toFixed(0);

                var selectMissedVotesPct = document.createElement("TD");
                if (array1[i].votes_with_party_pct !== 0) {
                    selectMissedVotesPct.innerHTML = array1[i].votes_with_party_pct + "%";
                } else {
                    selectMissedVotesPct.innerHTML = array1[i].votes_with_party_pct;
                }
